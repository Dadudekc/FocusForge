...
from PyQt5.QtWidgets import QDialog, QPushButton, QVBoxLayout, QFormLayout, QSpinBox, QCheckBox, QMessageBox
...
class SettingsDialog(QDialog):
    ...
    def toggle_dark_mode(self, state):
        if state == Qt.Checked:
            self.parent().setStyleSheet("QMainWindow { background-color: #2E2E2E; color: white; } QPushButton { background-color: #444444; color: white; } QLabel { color: white; }")
        else:
    ...
    def save_settings(self):
        ...
        if self.rl_toggle.isChecked() and not self.decision_engine.model:
            # Optionally, prompt to train or load a model
            ...
            # Optionally, disable RL features
